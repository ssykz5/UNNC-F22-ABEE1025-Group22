# Author: Haonan Zhang (20411814)
# Date of first creation: 2022-11-16
...
class coursework(object):
    ...
    def __init__(self, eplus_run_path, idf_path, output_dir,
        ...
        """
        init coursework class, must init:
                 eplus_run_path, idf_path, output_dir,
                 parameter_key1, parameter_key1_vals,
                 parameter_key2, parameter_key2_vals
        """
        ...
        self.parameter_key1 = parameter_key1
        self.parameter_key1_vals = parameter_key1_vals
        self.parameter_key2 = parameter_key2
        self.parameter_key2_vals = parameter_key2_vals

        self.full_parameter_values1 = []
        self.full_parameter_values2 = []
        self.output_paths = {}


    def run_two_simulation_helper(self, output_dir, parameter_key1_value, parameter_key2_value):
        """
        ...
        for i in range(len(self.parameter_key1)):
            if i < len(self.parameter_key1) - 1:
                inner_dict = inner_dict[self.parameter_key1[i]]
        inner_dict[self.parameter_key1[-1]] = parameter_key1_value

        # Change the JSON dict value of parameter_key2
        inner_dict = epjson_dict
        for j in range(len(self.parameter_key2)):
            if j < len(self.parameter_key2) - 1:
                inner_dict = inner_dict[self.parameter_key2[j]]
        inner_dict[self.parameter_key2[-1]] = parameter_key2_value
        ...
        # Run simulation
        run_eplus_model(self.eplus_run_path, self.idf_path, output_dir)

        # Return the path of the eplusout.csv file of this simulation
        return output_dir + '/eplusout.csv'

    def generate_parameter_values(self, parameter_vals, interval):
        """
    ...
    def run_two_parameter_parametric(self, test_interval_1, test_interval_2):
        ...
        self.full_parameter_values1 = self.generate_parameter_values(self.parameter_key1_vals, test_interval_1)
        self.full_parameter_values2 = self.generate_parameter_values(self.parameter_key2_vals, test_interval_2)

        # Initialize running time
        time = 1

        # Run simulation in order
        for parameter_key1_value in self.full_parameter_values1:

            for parameter_key2_value in self.full_parameter_values2:
                # Change the output_dir of each parameter value
                each_output_dir = self.output_dir + f"/run_{time}"

                output_path = self.run_two_simulation_helper(each_output_dir, parameter_key1_value,
                ...
                self.output_paths[this_key] = output_path
    ...
    def calculate_average_indoor_air_temperature(self):
        ...
        for path_key in self.output_paths.keys():
            this_mean_temperature = self.calculate_one_sim_avg_indoor_air_temperature(self.output_paths[path_key])
